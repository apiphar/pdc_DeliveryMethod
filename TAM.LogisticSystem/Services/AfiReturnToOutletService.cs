...
{
    /// <summary>
    /// Afi Return To Outlet
    /// </summary>
    public class AfiReturnToOutletService
    {
        private readonly LogisticDbContext LogisticDbContext;
        private readonly WebEnvironmentService WebEnvironmentService;

        public AfiReturnToOutletService(LogisticDbContext logisticDbContext,WebEnvironmentService webEnvironmentService)
        {
        ...
        public async Task<IEnumerable<AfiROGridView>> GetRequestByFilter(AfiRequestRevisiSearchRO model)
        {
            var query = @"SELECT [AfiApplicationId] = a.AfiApplicationId,[DODate]=h.IssuedDate,
                        ...
                        AND  a.AFIApplicationProcessEnumId IN(2,6,10) AND
                        ((@tanggalPengajuan IS NULL AND @sampai IS NULL)  
                        OR 
            ...
            return await this.LogisticDbContext.QueryAsync<AfiROGridView>(query, new
            {
        ...
        public async Task<IEnumerable<AfiROGridView>> GetRequest(AfiRequestRevisiSearchRO model)
        {
        ...
        public async Task<IEnumerable<AfiROGridView>> GetRequestByFrameNo(string FrameNumber)
        {
            var query = @"SELECT [AfiApplicationId] = a.AfiApplicationId,[DODate]=h.IssuedDate,
	                        [VehicleId] = a.VehicleId,[FrameNumber]=c.FrameNumber,[ModelCode]=f.CarModelCode,[ModelName]=f.Name,[Color]=a.Warna,[Jenis]=a.Jenis,[Model]=a.Model,[Chassis] = a.ChassisModel,
	                        [ApplicationNumber] = a.ApplicationNumber,[Branch] = (a.AFIBranchCode+' - '+l.Name), 
                        ...
                        JOIN BranchLocationMapping m ON m.BranchCode = j.BranchCode
                        WHERE b.AFIApplicationId IS NULL AND h.CancelledAt IS NOT NULL
                        AND  a.AFIApplicationProcessEnumId IN(2,6,10) AND FrameNumber = @FrameNumber
                        AND m.LocationCode = @LocationCode";
            return await this.LogisticDbContext.QueryAsync<AfiROGridView>(query, new { FrameNumber = FrameNumber,LocationCode = this.WebEnvironmentService.UserLocation });
        }
    }
}
