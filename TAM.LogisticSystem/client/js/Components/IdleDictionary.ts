...
class BreakHour {
    BreakHourTemplate: BreakHourTemplate;
    BreakHourTemplatesDetail: Array<BreakHourTemplateDetail>;
}
class BreakHourTemplate {
    breakHourTemplateCode: string;
    description: string;
}

class BreakHourTemplateDetail {
    breakHourTemplateCode: string;
    breakHourTemplateDetailId?: number;
    shiftCode: Shift;
    timeStart: string;
    timeFinish: string;
    isMonday: boolean;
...
export class IdleDictionaryController implements angular.IController {
    static $inject = ['IdleDictionaryServices', '$scope'];

    $scope: ng.IScope;



    breakHourTemplateService: Service.IdleDictionaryServices;
    searchString = {};
    loading: boolean;
    //all databaseData
    headerData: any;
    ...
    constructor(IdleDictionaryServices: Service.IdleDictionaryServices, protected _$scope: ng.IScope) {
        this.$scope = _$scope;
        this.breakHourTemplateService = IdleDictionaryServices;
    }
    ...
    reset(Form: angular.IFormController) {
        ...
        this.breakHourTemplateCode = null;
    ...
    refresh() {
        this.loading = true;
        this.breakHourTemplateService.getAll().then(response => {
            this.loading = false;
            this.headerData = response.data["breakHourTemplates"];
            this.detailData = response.data["breakHourTemplatesDetail"];
    ...
    checkHeader() {
        let data: any = null;
        data = Lodash.find(this.headerData, ['breakHourTemplateCode', this.breakHourTemplateCode]);
        if (data !== undefined) {
    ...
    changeDetailState() {
        ...
        if (!this.breakHourTemplateCode) {
            Alertify.error("Kode Pola harus diisi");
            check = false;
        }
        if (this.breakHourTemplateCode.length > 8) {
            Alertify.error("Kode Pola tidak boleh melebihi 8 karakter");
            check = false;
        }
        if (!regexp.test(this.breakHourTemplateCode)) {
    ...
    getDetail() {
        ...
        let data: any = Lodash.filter(this.detailData, ['breakHourTemplateCode', this.breakHourTemplateCode]);
        if (data !== null) {
            let order: number = 1;
    ...
    selectEditHeader(data) {
        this.breakHourTemplateCode = data.breakHourTemplateCode;
        this.description = data.description;
        this.addHeaderState = false;
        this.lock = true;
        this.detailState = false;
    ...
    selectDeleteHeader(data, Form: angular.IFormController) {
        let json = {}
        json["Kode Pola"] = data.breakHourTemplateCode;
        ...
        Alertify.confirm(
            ...
            () => {
                this.breakHourTemplateService.deleteData(data.breakHourTemplateCode).then(response => {
                    Alertify.success("Data header berhasil dihapus");
                }).catch(response => {
    ...
    addData(form: angular.IFormController) {
        ...
        //--------------------------------------------------------
        //init
        let dataPost: BreakHour = new BreakHour();
        dataPost.BreakHourTemplatesDetail = new Array<BreakHourTemplateDetail>();
        //handle header data
        dataPost.BreakHourTemplate = new BreakHourTemplate();
        dataPost.BreakHourTemplate.breakHourTemplateCode = this.breakHourTemplateCode;
        dataPost.BreakHourTemplate.description = this.description;
        //handle detail
        Lodash.forEach(this.tempDetail, (detail) => {
            let tempDetail: BreakHourTemplateDetail = new BreakHourTemplateDetail();            
            tempDetail.breakHourTemplateCode = this.breakHourTemplateCode;
            ...
            dataPost.BreakHourTemplatesDetail.push(tempDetail);
        });
        let json = {}
        json["Kode Pola"] = dataPost.BreakHourTemplate.breakHourTemplateCode;
        json["Keterangan"] = dataPost.BreakHourTemplate.description;
        Alertify.confirm(
            "Konfirmasi",
            Mustache.render(require("./alertify/MasterAlertify.html"), this.convertToMustacheJSON("insert", json)),
            () => {
                this.breakHourTemplateService.postDataBreak(dataPost).then(response => {
    ...
    gotoGenerate() {
        window.location.href = '/generatejambreak';
    }

}


export class IdleDictionaryComponent implements angular.IComponentOptions {
    controller = IdleDictionaryController;
    controllerAs = 'me';

    template = require('./IdleDictionary.html');
